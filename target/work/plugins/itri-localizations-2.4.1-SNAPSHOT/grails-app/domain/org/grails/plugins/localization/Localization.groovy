...
class Localization implements Serializable {
    ...
    static mapping = Holders.config.grails.plugin.localizations.mapping ?: {
        columns {
            ...
            locale column: "loc"
            text type : 'text' // smallufo : move text to here
        }
    }
    ...
    static String decodeMessage(String code, String locale) {
        return decodeMessage(code,new Locale(locale))
    }
    ...
    def beforeValidate(){
        if(locale.indexOf("_")>0) locale = locale.replace("_","")
    }
    def beforeInsert() {
        if(locale.indexOf("_")>0) locale = locale.replace("_","")
    }

    static Localization findInstanceByCodeAndLocale(String code , String locale){
        def instance = Localization.findByCodeAndLocale(code,locale.replace('_',''))
        return instance;
    }
    //取得多國語系內容
    public static String getMessage(String code,String locale="*"){
        if(locale.indexOf("_")>0) locale = locale.replace("_","")
        def message = findInstanceByCodeAndLocale(code,locale)
        if(!message)message = findInstanceByCodeAndLocale(code,'*')
        if(!message){
            return code
        }else{
            return message.text
